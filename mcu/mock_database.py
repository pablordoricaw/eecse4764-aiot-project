...
def generate_random_log(req_device_id):
    ...
    if is_temp_error:
        ...
        error_types = [
            (
                "E-TEMP-HIGH",
                f"Critical fault: Temperature threshold exceeded ({current_temp:.1f}°C).",
            ),
            (
                "E-HUMIDITY-HIGH",
                f"Critical fault: Humidity anomaly detected ({current_humidity:.1f}%).",
            ),
...
def get_logs():
    ...
    response = {"logs": [new_log], "next_since": new_log["timestamp"]}
    ...
    if new_log["level"] == "ERROR":
        ...
        print(
            f" >>> Current: Temp={current_temp:.1f}°C, Humidity={current_humidity:.1f}%"
        )
        print("!" * 60 + "\n")
    else:
